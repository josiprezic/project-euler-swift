...
//  project-euler-swift
//
//  Created by Josip Rezic on 11/01/2022.
//
...
struct Challenge011: Challenge {
    ...
    func solve() -> Int {
        ...
        let allArrays = matrix + transposedMatrix + rightDiagonals + leftDiagonals
    ...
    private func rightDiagonalArrays(from matrix: [[Int]]) -> [[Int]] {
        let minIndexSum = factorCount - 1
        let maxIndexSum = (size - factorCount) * 2
        let indexSumRange = Array(minIndexSum..<maxIndexSum)
        return indexSumRange.map { rightDiagonalArray(from: matrix, indexSum: $0) }
    }
    ...
    private func largestProduct(in array: [Int]) -> Int {
        let startIndexMax = array.count - factorCount
        let startIndexRange = 0...startIndexMax
        
        return startIndexRange
            .map { array[$0..<($0 + factorCount)] }
            .map { $0.reduce(1, *) }
            .sorted()
            .last ?? 0
    }
